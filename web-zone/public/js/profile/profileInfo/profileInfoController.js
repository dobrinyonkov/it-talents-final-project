(function () {
    app.controller("ProfileInfoController", function (
        ...
        $scope.newProfilePic = { url: "", uploaded: false, busy: false };
        $scope.newCoverPhoto = { url: "", uploaded: false, busy: false };
        ...
        $scope.saveAcount = function(user, url, isCover) {
            console.log(user)
          if (isCover) {
            user.coverPhoto = url;
          } else {
          ...
          UserService.update(user)
            .then(r => {
              ...
              $scope.newCoverPhoto.uploaded = false;
            })
            .catch(err => {
              console.log(err);
        ...
        var selectedFile2 = document.getElementById("selectedFile2");
        ...
        selectedFile2.addEventListener("change", function(event) {
          ...
          if (!allowedExtensions.exec(file.name)) {
            ...
            fileUpload
              .uploadFileToUrl(file)
              .then(r => {
                console.log("scope.profile after loading cover photo");
                console.log($scope.profile)
                $scope.profile.coverPhoto = r.data.url;
                $scope.newCoverPhoto.uploaded = true;
                $scope.newCoverPhoto.url = r.data.url;
                $scope.newCoverPhoto.busy = false;
              })
              .catch(err => {
                console.log(err);
                alert("We coudn't upload this image, please try again later, or with a different image.");
        ...
        $scope.sendFriendRequest = function(senderId, receiverId) {
          ...
          if (!$scope.areFriends && !$scope.friendsRequestSended) {
            ...
            if ($scope.friendsRequestSended && !$scope.areFriends) {
              UserService.friendsRequests
                .deleteFR(senderId, receiverId)
                .then(r => {
                  $scope.friendsRequestSended = false;
                  var index = $scope.sendedFriendRequests.findIndex(
                    function(user) {
              ...
              UserService.friendsRequests
                ...
                .then(r => {
                  ...
                  $scope.friendsRequestSended = false;
                  var index = $rootScope.currentUser.friends.findIndex(
                    function(user) {
                      return user._id === senderId;
                    }
                  );
