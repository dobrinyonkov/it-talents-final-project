...
(function() {
  ...
  document
    ...
    .addEventListener("wheel", function(e) {
      if (this.scrollTop + this.clientHeight >= this.scrollHeight - 80) {
        ...
        if ($scope.posts.busy) {
          console.log("please wait , I'm loading a post");
          return;
        }
  ...
  // DISPLAY POST FUNCTION -loads the post and augments it with flags and methods that are view binded
  $scope.posts.displayPost = function(postId, top) {
    return PostService.getPost(postId)
      .then(post => post.loadOwnerInfo()) // POST OWNER INFO
      .then(post => {
        // DELETING A POST
        ...
        // LIKING A POST
        post.liked =
          post.likes.indexOf(localStorage.getItem("loggedUserId")) != -1; //liked by that user
